...
failed:
	DEBUG_MSG("[ERROR] Alloc memory for OGG_DEC_HANDLE failed\n");
...
int ogg_decode_initial(OGG_DEC_HANDLE *handle, unsigned char *ogg_data, int *used_size)
{
	...
	if (handle->mState == NULL) {
		DEBUG_MSG("[ERROR] Alloc memory for vorbis_dsp_state failed\n");
		return FALSE;
	}
	...
	if (handle->mVi == NULL) {
		DEBUG_MSG("[ERROR] Alloc memory for vorbis_info failed\n");
		free(handle->mState);
		return FALSE;
	}
